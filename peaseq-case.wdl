...
### ------------------------------------------------- ###
    ...
    scatter (eachfastq in all_sample_fastqfiles) {
        call fastqc.fastqc as indv_fastqc {
            input :
                inputfile=eachfastq,
                default_location=if multi_fastqpair then 'SAMPLE/' + sub(basename(eachfastq),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/QC/FastQC' else if defined(results_name) then results_name + '/single-end_mode/QC/FastQC' else sub(basename(eachfastq),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/QC/FastQC'
        }

        call util.basicfastqstats as indv_bfs {
            input :
                fastqfile=eachfastq,
                default_location=if multi_fastqpair then 'SAMPLE/' + sub(basename(eachfastq),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/QC/SummaryStats' else if defined(results_name) then results_name + '/single-end_mode/QC/SummaryStats' else sub(basename(eachfastq),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/QC/SummaryStats'
        }
        ...
        call mapping.mapping as indv_mapping {
            input :
                ...
                blacklist=blacklist,
                default_location=if multi_fastqpair then 'SAMPLE/' + sub(basename(eachfastq),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/BAM_files' else if defined(results_name) then results_name + '/single-end_mode/BAM_files' else sub(basename(eachfastq),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/BAM_files'
        }
    ...
    call samtools.mergebam as SE_mergebam {
        input:
            ...
            default_location = if defined(results_name) then results_name + '/single-end_mode/BAM_files' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/BAM_files' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/BAM_files',
    ...
    call fastqc.fastqc as SE_mergebamfqc {
        input:
            ...
            default_location=if defined(results_name) then results_name + '/single-end_mode/QC/FastQC' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/QC/FastQC' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/QC/FastQC'
    }
        ...
        scatter (fastqpair in sample_fastqfiles) {
            ...
            call mapping.mapping as indv_PE_mapping {
                input :
                    ...
                    blacklist=blacklist,
                    paired_end=true,
                    default_location='SAMPLE/' + sub(basename(fastqpair.left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/BAM_files'
            }

            call fastqc.fastqc as indv_PE_bamfqc {
                input :
                    inputfile=indv_PE_mapping.sorted_bam,
                    default_location='SAMPLE/' + sub(basename(fastqpair.left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/QC/FastQC'
            }
            ...
            call util.evalstats as indv_PE_summarystats {
                input:
                    ...
                    default_location='SAMPLE/' + sub(basename(fastqpair.left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/QC/SummaryStats'
            }
    ...
    if ( one_fastqpair ) {
        ...
        call mapping.mapping as uno_PE_mapping {
            input :
                ...
                default_location=if defined(results_name) then results_name + '/BAM_files' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/BAM_files'
        }
...
### ------------------------------------------------- ###
    ...
    call macs.macs as SE_all {
        input :
            ...
            keep_dup="all",
            egs=egs.genomesize,
            default_location = if defined(results_name) then results_name + '/single-end_mode/PEAKS/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '-p9_kd-all' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/PEAKS/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '-p9_kd-all' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/PEAKS/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '-p9_kd-all',
            coverage_location = if defined(results_name) then results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '_p9_kd-all' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '_p9_kd-all' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '_p9_kd-all'
    }
    ...
    call macs.macs as SE_nomodel {
        input :
            ...
            nomodel=true,
            egs=egs.genomesize,
            default_location = if defined(results_name) then results_name + '/single-end_mode/PEAKS/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '-nm' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/PEAKS/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '-nm' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/PEAKS/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '-nm',
            coverage_location = if defined(results_name) then results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '_nm' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '_nm' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + basename(SE_mergebam_afterbklist,'.bam') + '_nm'
    }
    ...
    call bamtogff.bamtogff as SE_bamtogff {
        input :
            ...
            default_location = if defined(results_name) then results_name + '/single-end_mode/BAM_Density' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/BAM_Density' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/BAM_Density'
    }
    ...
    call sicer.sicer as SE_sicer {
        input :
            ...
            default_location = if defined(results_name) then results_name + '/single-end_mode/PEAKS/BROAD_peaks' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/PEAKS/BROAD_peaks' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/PEAKS/BROAD_peaks',
            coverage_location = if defined(results_name) then results_name + '/single-end_mode/COVERAGE_files/BROAD_peaks' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/COVERAGE_files/BROAD_peaks' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/COVERAGE_files/BROAD_peaks'
    }
    ...
    call rose.rose as SE_rose {
        input :
            ...
            default_location = if defined(results_name) then results_name + '/single-end_mode/PEAKS/STITCHED_peaks' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/PEAKS/STITCHED_peaks' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/PEAKS/STITCHED_peaks'
    }
    ...
    call util.peaksanno as SE_sicer_peaksanno {
        input :
            ...
            chromsizes=samtools_faidx.chromsizes,
            default_location = if defined(results_name) then results_name + '/single-end_mode/PEAKS_Annotation/BROAD_peaks' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/PEAKS_Annotation/BROAD_peaks' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/PEAKS_Annotation/BROAD_peaks'
    }
    ...
    if (run_motifs) {
        call motifs.motifs as SE_motifs {
            input:
                ...
                default_location = if defined(results_name) then results_name + '/single-end_mode/MOTIFS' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/MOTIFS' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/MOTIFS'
        }
    ...
    call viz.visualization as SE_vizall {
        input:
            ...
            default_location = if defined(results_name) then results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + sub(basename(SE_all.peakbedfile),'_peaks.bed','') else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + sub(basename(SE_all.peakbedfile),'_peaks.bed','') else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + sub(basename(SE_all.peakbedfile),'_peaks.bed','')
    }
    ...
    call viz.visualization as SE_viznomodel {
        input:
            ...
            default_location = if defined(results_name) then results_name + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + sub(basename(SE_nomodel.peakbedfile),'_peaks.bed','') else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + sub(basename(SE_nomodel.peakbedfile),'_peaks.bed','') else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/COVERAGE_files/NARROW_peaks' + '/' + sub(basename(SE_nomodel.peakbedfile),'_peaks.bed','')
    }
    ...
    call viz.visualization as SE_vizsicer {
        input:
            ...
            chromsizes=samtools_faidx.chromsizes,
            default_location = if defined(results_name) then results_name + '/single-end_mode/COVERAGE_files/BROAD_peaks' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/COVERAGE_files/BROAD_peaks' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/COVERAGE_files/BROAD_peaks'
    }
...
### ------------------------------------------------- ###
    call util.evalstats as SE_summarystats {
        input:
            ...
            default_location = if defined(results_name) then results_name + '/single-end_mode/QC/SummaryStats' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode/QC/SummaryStats' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode/QC/SummaryStats'
    }
    ...
    call util.summaryreport as merge_overallsummary {
        input:
            ...
            default_location=if defined(results_name) then results_name + '/single-end_mode' else if multi_fastqpair then 'AllMapped_' + length(sample_fastqfiles) + 'fastqpairs' + '/single-end_mode' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '/single-end_mode'
    }
### ------------------------------------------------- ###
### ----------------- S E C T I O N 5 --------------- ###
### --------------- Summary Statistics -------------- ###
    ...
    if ( one_fastqpair ) {
        ...
        call peaseq_util.pairedend_summaryreport as PE_uno_overallsummary {
            ...
            input:
                ...
                outputfile = if defined(results_name) then results_name + '.peaseq_report.html' else sub(basename(sample_fastqfiles[0].left),'_R?[12]_?[:digit:]{0,3}.f[:alpha:]{0,3}q.gz','') + '.peaseq_report.html'
        }
