...
def main(reorg_conf___=None, reorg_status___=None):
    ...
    common_map = [
        x["execution"]["output"] for x in stages if x["id"] == "stage-common"
    ][0]

    folder_location = [
        x["execution"]["folder"] for x in stages if x["id"] == "stage-outputs"
    ][0]
    ...
    for eachfile in dx_container.list_folder(folder_location)["objects"]:
        dx_container.move(destination=temp_folder, objects=[eachfile["id"]])
        ...
        if isinstance(file_id_information, list):
            for indvfile in range(0, len(file_id_information)):
                default_location = dxpy.describe(
                    dxpy.describe(file_id_information[indvfile])["createdBy"]["job"]
