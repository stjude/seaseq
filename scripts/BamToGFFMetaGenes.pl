#!/usr/bin/perl
# processing BamToGFF to calculate MetaGenes
use Pod::Usage;
use strict; 
use warnings;
use File::Basename;
...
`rm -rf *gff`; #remove the gff files
...
foreach my $index (qw|promoters upstream downstream genebody|){ `rm -rf $outfile-sorted.$index.txt`; }
